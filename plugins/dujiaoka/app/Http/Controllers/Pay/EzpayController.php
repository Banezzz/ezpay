...
class EzpayController extends PayController
{
    public function gateway(string $payway, string $orderSN)
    {
        try {
            ...
            $parameter = [
                ...
                'redirect_url' => route('ezpay-return', ['order_id' => $this->order->order_sn]),
                'notify_url' => url($this->payGateway->pay_handleroute . '/notify_url'),
            ];
            $parameter['signature'] = $this->ezpaySign($parameter, $this->payGateway->merchant_id);
            $client = new Client([
    ...
    private function ezpaySign(array $parameter, string $signKey)
    {
        ksort($parameter);
    ...
    public function notifyUrl(Request $request)
    {
        ...
        $signature = $this->ezpaySign($data, $payGateway->merchant_id);
